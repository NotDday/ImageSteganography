...
def decode():
    image = Image.open(img_path, 'r')
    data = ''
    imdat = iter(image.getdata())
    while (True):
        pixels = [value for value in imdat.__next__()[:3]+imdat.__next__()[:3]+imdat.__next__()[:3]]
        binstr = ''
        for i in pixels[:8]:
            if (i % 2 == 0):
                binstr += '0'
...
sel_btn = PhotoImage(file=relative_to_assets("file.png"))
image_btn = Button(sf,image=sel_btn,command=select_img,cursor="target",relief='flat')
...
txt_btn = Button(ef,image=sel_btn,borderwidth=0,highlightthickness=0,command=select_txt,activebackground="#202020",cursor="target",relief='flat')
txt_btn.place(x=402.0,y=130.4,width=16.0,height=15.2)
# Input Text
txt_box = Text(ef,bd=0,bg="#2D2D2D",fg="white",font=("Roboto Medium", 14 * -1),padx=5,pady=5)
...
# Text Selection Button
save_sel = Button(df,image=sel_btn,command=select_save,activebackground="#202020",cursor="target")
...
msg_box = Text(df,bd=0,bg="#2D2D2D",fg="white",font=("Roboto Medium", 14 * -1),padx=5,pady=5)
...
decode_btn = Button(sf,image=decode_btn_image,command=lambda : handle_btn_press("df"),activebackground= "#202020")
decode_btn.place(x=235.0,y=220.0,width=205.0,height=47.0)
# Encode Frame Button
ef_button_image = PhotoImage(file=relative_to_assets("ef.png"))
ef_btn = Button(sf,image=ef_button_image,command=lambda : handle_btn_press("ef"),activebackground= "#202020")
